...
6. 使用冻结参数低速复测矩形循迹，不在移植过程中重调 PID；
7. 在另一控制器上单独验收 OpenMV/MG996R，不向 STM32 独立版增加视觉或云台链路。
...
遥测周期为 200ms。若增加日志字段或提高输出频率，应通过
...
通常是供电跌落或电流不足。独立版不在 STM32 与 OpenMV 之间连接数据线；OpenMV/MG996R 应按硬件要求独立供电，不能从 STM32 3.3V 引脚取大电流。
